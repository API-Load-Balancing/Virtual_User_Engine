...
module.exports = function (userOptions) {  // constructor method
    ...
    this.AddRequestTimeoutMiddleware = function (middleware) {

        if (!middleware) return;
        if (typeof middleware === "function") {
            if (options.requestTimeoutMiddlewareList.indexOf(middleware) >= 0) return;
            options.requestTimeoutMiddlewareList.push(middleware);
        }
    }


    this.removeRequestTimeoutMiddleware = function (middleware) {
        if (!middleware) return;
        if (typeof middleware === "function") {
            var placeInArray = options.requestTimeoutMiddlewareList.indexOf(middleware);
            if (placeInArray >= 0) options.requestTimeoutMiddlewareList.splice(placeInArray, 1);
        }
    }
    ...
    this.setFilteredModuleList = function (filterList) {
        if (typeof filterList === 'object' && Array.isArray(filterList))
            options.filteredModuleList = filterList.slice(0);
    }
    ...
    this.addFilteredModule = function (moduleName) {
        if (typeof moduleName === 'string' || moduleName instanceof String) {
            var placeInArray = options.filteredModuleList.indexOf(moduleName);
            if (placeInArray >= 0) options.filteredModuleList.push(moduleName);
        }
    }
    ...
    this.removeFilteredModule = function (moduleName) {
        if (typeof moduleName === 'string' || moduleName instanceof String) {
            var placeInArray = options.filteredModuleList.indexOf(moduleName);
            if (placeInArray >= 0) options.filteredModuleList.splice(placeInArray, 1);
        }
    }
